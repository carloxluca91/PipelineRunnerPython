...
class TestCreateColumn(AbstractTestCase):
    ...
    def test_random_column(self):
        ...
        from pipeline_runner.pypeline.create.column import RandomColumn
        ...
        for index, column in enumerate(self.column_list):
            ...
            if column_type_lc not in ["timestamp", "date"]:
                ...
                self._logger.info(f"Analyzing column # {index}, '{random_column.name}'")
                ...
                self._logger.info(f"Successfully analyzed column # {index}, '{random_column.namee}'")
